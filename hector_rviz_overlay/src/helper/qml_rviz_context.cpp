...
QmlRvizContext::QmlRvizContext( rviz_common::DisplayContext *context, const Overlay *overlay,
                                bool visible )
    : context_( context ), overlay_( overlay ), visible_( visible )
{
  ...
  connect( context_->getFrameManager(), &rviz_common::FrameManagerIface::fixedFrameChanged, this,
           &QmlRvizContext::fixedFrameChanged );
...
bool QmlRvizContext::visible() const { return visible_; }
...
void QmlRvizContext::setIsFullscreen( bool value )
{
  ...
  rviz_common::WindowManagerInterface *wmi = context_->getWindowManager();
  if ( wmi == nullptr ) {
    LOG_ERROR( "setIsFullscreen failed: Window could not be obtained! Implement the "
               "WindowManagerInterface in the DisplayContext." );
    return;
  }
  // Unfortunately this method is not exposed in the public interface, hence, we need to work around this using Qt
  QMetaObject::invokeMethod( context_->getWindowManager()->getParentWindow(), "setFullScreen",
                             Q_ARG( bool, value ) );
}

QString QmlRvizContext::fixedFrame() const { return context_->getFixedFrame(); }
...
{
  if ( ( state & Qt::WindowFullScreen ) != ( window_state_ & Qt::WindowFullScreen ) )
...
void QmlRvizContext::setConfigurationPropertyParent( Property *parent )
{
  ...
  if ( parent != nullptr )
    parent->addChild( configuration_property_ );
}
...
{
  if ( path.empty() ) {
    if ( !config.isValid() || config.getType() != rviz_common::Config::Value )
      return {};
    return config;
  }
  if ( !config.isValid() || config.getType() != rviz_common::Config::Map )
    return {};
...
QObject *QmlRvizContext::registerBoolProperty( const QString &name, bool default_value,
                                               const QString &description )
{
  QmlRvizProperty tmp( configuration_property_ );
...
QObject *QmlRvizContext::registerBoolProperty( QmlRvizProperty *parent, const QString &name,
                                               bool default_value, const QString &description )
{
  auto *prop = findChildProperty( parent->property(), name );
  if ( prop == nullptr ) {
...
QObject *QmlRvizContext::registerFloatProperty( QmlRvizProperty *parent, const QString &name,
                                                float default_value, const QString &description )
{
  auto *prop = findChildProperty( parent->property(), name );
  if ( prop == nullptr ) {
                                                  ...
                                                  bool include_fixed_frame )
{
  auto *prop = findChildProperty( parent->property(), name );
  if ( prop == nullptr ) {
    prop = new TfFrameProperty( name, default_value, description, parent->property(), nullptr,
                                include_fixed_frame );
    loadPropertyFromConfig( prop );
  }
  return new QmlRvizProperty( prop );
