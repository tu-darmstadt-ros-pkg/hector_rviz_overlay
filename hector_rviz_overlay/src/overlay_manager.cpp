...
OverlayManager::OverlayManager()
    : context_( nullptr ), render_panel_( nullptr ), renderer_( nullptr ),
      mouse_overlay_( nullptr ), mouse_down_overlay_( nullptr ), focused_overlay_( nullptr )
{
}

OverlayManager::~OverlayManager() = default;

rviz_common::DisplayContext *OverlayManager::displayContext() { return context_; }
...
void OverlayManager::init( rviz_common::DisplayContext *context )
{
  ...
  connect( QCoreApplication::instance(), &QCoreApplication::aboutToQuit, this,
           &OverlayManager::onAboutToQuit, Qt::DirectConnection );
}
...
void OverlayManager::onZIndexChanged()
{
  ...
  auto element = std::find_if( ui_overlays_.begin(), ui_overlays_.end(),
                               [=]( const OverlayPtr &x ) { return x.get() == overlay; } );
  if ( element == ui_overlays_.end() )
    return;
...
bool OverlayManager::eventFilter( QObject *receiver, QEvent *event )
{
  ...
  if ( !renderer_->isRendering() )
    return false;
  if ( !render_panel_->isActiveWindow() )
    return false; // TODO actually we just need to know if there is a dialog in front
...
bool OverlayManager::handleMouseEvent( QObject *receiver, QMouseEvent *event )
{
  ...
  // If the mouse is down, not just pressed and the pressed wasn't handled by the overlay, ignore
  // subsequent events until mouse is up again.
  if ( is_mouse_down &&
...
{
  // Map the wheel event to the render panel because the top left of the render panel is equivalent
  // to the top left of the overlays.
  QPoint render_panel_pos = render_panel_->mapFromGlobal( event->globalPosition().toPoint() );
  if ( !render_panel_->contentsRect().contains( render_panel_pos ) )
    return false;

  QWheelEvent child_event( render_panel_pos, event->globalPosition(), event->pixelDelta(),
