...
QWidget *PopupContainerWidget::popup() { return popup_; }

const QWidget *PopupContainerWidget::popup() const { return popup_; }
...
bool PopupContainerWidget::isModalPopup() const { return is_modal_; }

void PopupContainerWidget::setIsModalPopup( bool value ) { is_modal_ = value; }

const QColor &PopupContainerWidget::modalColor() const { return modal_color_; }

void PopupContainerWidget::setModalColor( const QColor &color ) { modal_color_ = color; }

AnchorPoint PopupContainerWidget::anchorPoint() const { return anchor_point_; }

void PopupContainerWidget::setAnchorPoint( AnchorPoint value ) { anchor_point_ = value; }
...
QPoint PopupContainerWidget::popupPosition() const { return popup_->pos(); }
...
void PopupContainerWidget::setPopupPosition( const QPoint &position )
{
  ...
  if ( x < 0 )
    x = 0;
  else if ( x > geometry().width() - popup_->width() )
    x = geometry().width() - popup_->width();
  int y = position.y();
  if ( y < 0 )
    y = 0;
  else if ( y > geometry().height() - popup_->height() )
    y = geometry().height() - popup_->height();
